package ch.zhaw.simulation.sysintegration.mac.ui;
...
/**
 * Based on: http://code.google.com/p/macwidgets/
 * 
...
public class FlatToolbarButtonUI extends BasicButtonUI {
	...
	protected void paintIcon(Graphics g, JComponent c, Rectangle iconRect) {
		...
		BufferedImage image = new BufferedImage(iconRect.width, iconRect.height, BufferedImage.TYPE_INT_ARGB);
	...
	protected void paintText(Graphics g, JComponent c, Rectangle textRect, String text) {
		...
		graphics.setColor(model.isArmed() ? EMPTY_COLOR : DEFAULT_EMPHASIS_COLOR);
		BasicGraphicsUtils.drawStringUnderlineCharAt(graphics, text, -1, textRect.x, textRect.y + 1 + fm.getAscent());

		// 2) Draw the text.
		graphics.setColor(model.isEnabled() ? DEFAULT_FOCUSED_FONT_COLOR : DEFAULT_DISABLED_FONT_COLOR);
