...
/**
 * A task that may be executed in a separate thread and permits examination of
 * its status. Other threads can check if it is finished or wait for it to
 * finish.
 * <P>
 * For example:
 * <p>
 ...
 * </PRE></code>
 * <p>
 * In a different thread one can then test <CODE>task.isFinished ()</CODE> or
 * wait for it with <CODE>task.waitFinished ()</CODE>.
 * 
...
public class Task extends Object implements Runnable {
	 ...
	 * that do not override waitFinished (long)
	 */
	...
	/**
	 * Create a new task. The runnable should provide its own error-handling, as
	 * by default thrown exceptions are simply logged and not rethrown.
	 * 
	 * @param run
	 *            runnable to run that computes the task
	 */
	...
	/**
	 * Wait until the task is finished. Changed not to be <code>final</code> in
	 * version 1.5
	 */
	...
	/**
	 * Wait until the task is finished, but only a given time. waitFinished(0)
	 * means indefinite timeout (similar to wait(0))
	 * 
	 * @param milliseconds
	 *            time in milliseconds to wait for the result
	 * @exception InterruptedException
	 *                when the waiting has been interrupted
	 * @return true if the task is really finished, or false if the time out has
	 *         been exceeded
	 * @since 5.0
	 */
	...
	/**
	 * Changes the state of the task to be running. Any call after this one and
	 * before notifyFinished to waitFinished blocks.
	 * 
	 * @since 1.5
	 */
	...
	/**
	 * Start the task. When it finishes (even with an exception) it calls
	 * {@link #notifyFinished}. Subclasses may override this method, but they
	 * then need to call {@link #notifyFinished} explicitly.
	 * <p>
	 * Note that this call runs synchronously, but typically the creator of the
	 * task will call this method in a separate thread.
	 */
	public void run() {
		try {
			notifyRunning();
	...
	/**
	 * Reveal the identity of the worker runnable. Used for debugging from
	 * RequestProcessor.
	 */
	String debug() {
