...
/**
 * A class that handles the "File Name:" text field auto-completion drop-down
 * selection list.
 * 
 * @author Soot Phengsy
 */
public class FileCompletionPopup extends JPopupMenu implements KeyListener {
	...
	private void setSelectPrevious() {
		if (list.getModel().getSize() > 0) {
			int cur = (list.getSelectedIndex() == -1) ? 0 : list.getSelectedIndex();
	...
	private class MouseHandler extends MouseAdapter implements MouseMotionListener {
		...
		private MouseEvent convertMouseEvent(MouseEvent e) {
			...
			MouseEvent newEvent = new MouseEvent((Component) e.getSource(), e.getID(), e.getWhen(), e.getModifiers(), convertedPoint.x, convertedPoint.y,
	...
	private boolean isCompletionKey(int keyCode, JTextField textField) {
		...
		if (keyCode == KeyEvent.VK_RIGHT && (textField.getCaretPosition() >= (textField.getDocument().getLength() - 1))) {
			return true;
		}
