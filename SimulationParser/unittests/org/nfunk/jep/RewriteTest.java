...
public class RewriteTest extends DJepTest {
	...
	public void testMemory() throws Exception {
		...
		taylorParser.addVariable("x", 0);
		/*
		 * try { Node node =taylorParser.parse(
		 * "diff(diff(diff(diff(diff(diff(diff(diff(ln(x+1),x),x),x),x),x),x),x),x)"
		 * ); Node processed = taylorParser.preprocess(node); Node simp =
		 * taylorParser.simplify(processed); } catch(OutOfMemoryError e) {
