 ...
 * Expressions from a text file are evaluated with JEP in pairs of two, and the
 * results are compared. If they do not match, the two expressions are printed
 * to standard output.
 * <p>
 ...
 * </pre>
 * 
 * The expressions '1+2' and '3' are evaluated with JEP and the results
 * compared.
 */
public class JEPEvaluationTest extends TestCase {
	 ...
	 * Constructor
	 * 
	 * public JEPTester() { // Set up the parser myParser = new JEP();
	 * myParser.setImplicitMul(true); myParser.addStandardFunctions();
	 * myParser.addStandardConstants(); myParser.addComplex();
	 * myParser.setTraverse(false); lineCount = 0; }
	 */

	/**
	 * Creates a new JEPTest instance
	 */
	public JEPEvaluationTest(String name) {
		super(name);
	}
	...
	/**
	 * The main method checks the arguments and creates an instance and calls
	 * it's run method.
	 */
	public static void main(String args[]) {
		...
		JEPEvaluationTest jt = new JEPEvaluationTest("JEP Test");
		jt.setUp();
	...
	/**
	 * Loads the file specified in fileName. Evaluates the expressions listed in
	 * it and compares the expressions with the results.
	 */
	public void testWithFile(String fileName) {
		...
		while (true) {
			...
			try {
				v1 = parseNextLine(reader); // returns null when end of file is
				// reached
	...
	/**
	 * Parses a single line from the reader, and returns the evaluation of that
	 * line.
	 * 
	 * @return evaluated line. Returns null when the end of the file is reached.
	 * @throws Exception
	...
	public void testGetComplexValue() {
		...
		myParser.parseExpression("2.1345");
		Assert.assertTrue(new Complex(2.1345, 0).equals(myParser.getComplexValue(), 0));
