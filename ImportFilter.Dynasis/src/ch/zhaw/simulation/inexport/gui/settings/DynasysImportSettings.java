...
public class DynasysImportSettings extends JPanel {
	...
	public DynasysImportSettings(Settings settings) {
		...
		setBorder(new TitledBorder("Dynasis Importeinstellungen"));
		
		gbm.setX(0).setY(0).setWeightY(0).setComp(new JLabel("Skalierungsfaktor"));
		gbm.setX(1).setY(0).setWeightY(0).setComp(txtScale);
		
		gbm.setX(0).setWidth(2).setY(1).setWeightY(0).setComp(getInfolabel("<html>Dynasys Symbole sind viel kleiner als unsere,<br>daher kann das ganze Dokument skaliert werden,<br>um es ordnungsgemäss darzustellen.</html>"));
