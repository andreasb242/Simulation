...
public class SJepTest extends TestCase {
	...
	/*
	 * public Node parseProcSimpEval(String expr,Object expected) throws
	 * ParseException,Exception { Node node = j.parse(expr); Node processed =
	 * j.preprocess(node); Node simp = j.simplify(processed); Object res =
	 * j.evaluate(simp);
	 * 
	 * if(!expected.equals(res))
	 * System.out.println("Error: Value of \""+expr+"\" is \""
	...
	public void testPolySimp() throws ParseException, Exception {
		...
		j.println(Q8expand);
		expandTestString("(xx^2+yy^2+zz^2+ww^2)^4", "24*ww^2*xx^2*yy^2*zz^2+12*ww^2*xx^2*yy^4+12*ww^2*xx^2*"
				+ "zz^4+12*ww^2*xx^4*yy^2+12*ww^2*xx^4*zz^2+4*ww^2*xx^6+" + "12*ww^2*yy^2*zz^4+12*ww^2*yy^4*zz^2+4*ww^2*yy^6+4*"
				+ "ww^2*zz^6+12*ww^4*xx^2*yy^2+12*ww^4*xx^2*zz^2+6*ww^4*" + "xx^4+12*ww^4*yy^2*zz^2+6*ww^4*yy^4+6*ww^4*zz^4+4*"
				+ "ww^6*xx^2+4*ww^6*yy^2+4*ww^6*zz^2+ww^8+12*xx^2*yy^2*" + "zz^4+12*xx^2*yy^4*zz^2+4*xx^2*yy^6+4*xx^2*zz^6+12*"
				+ "xx^4*yy^2*zz^2+6*xx^4*yy^4+6*xx^4*zz^4+4*xx^6*yy^2+4*" + "xx^6*zz^2+xx^8+4*yy^2*zz^6+6*yy^4*zz^4+4*yy^6*zz^2+" + "yy^8+zz^8");
	...
	public void testBad() throws ParseException {
		if (SHOW_BAD) {
			...
			simplifyTest("diff(re(x+i y),x)", "1"); // not smart enought to work
			// out re(i) = 1
