...
public class BugsTest extends TestCase {
	 ...
	 * 
	 * This bug turned out to actually not be a bug. The user reported that no
	 * error occured from a custom function during parsing, only after
	 * evaluation. This is expected behaviour since the run() method is not
	 * called during parsing - so even if there is a type compatibility issue,
	 * it will not be determined while parsing.
	 */
	public void testHasError() {
		...
		// test for empty expression causing error (should have error after
		// parsing)
		jep.parseExpression("");
		Assert.assertTrue(jep.hasError());
		jep.getValue();
	...
	/**
	 * Inner class for testing bug 1562371 This custom function returns the
	 * parameter if it is a regular number greater than zero. It throws an
	 * exception otherwise.
	 * 
	 * @author singularsys
	 */
	 ...
	 * 
	 * This test parses the expression "x" and checks whether only the variable
	 * x is in the symboltable (no more no less)
	 */
	 ...
	 * 
	 * public void testComplexPower() { jep.initSymTab();
	 * jep.parseExpression("(-8)^(1/3)"); Complex result =
	 * jep.getComplexValue(); Assert.assertTrue(result.equals(new Complex(-2,
	 * 0))); }
	 */
	 ...
	 * 
	 * JEP 2.4.0 checks the <code>errorList</code> variable before evaluating an
	 * expression if there is an error in the list, null is returned. This
	 * behaviour is bad because errors are added to the list by
	 * getValueAsObject. If the first evaluation fails (after a successful
	 * parse) then an error is added to the list. Subsequent calls to
	 * getValueAsObject fail because there is an error in the list.
	 */
