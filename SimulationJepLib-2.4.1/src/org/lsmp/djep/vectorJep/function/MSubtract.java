...
public class MSubtract extends Subtract implements BinaryOperatorI {
	...
	/** Adds two vectors. */
	private MVector sub(MVector lhs, MVector rhs) throws ParseException {
	...
	/** Adds two matrices. */
	private Matrix sub(Matrix lhs, Matrix rhs) throws ParseException {
	...
	/** Adds two matrices. */
	private Tensor sub(Tensor lhs, Tensor rhs) throws ParseException {
		if (lhs.getNumEles() != rhs.getNumEles())
			throw new ParseException("Miss match in sizes (" + lhs.getNumEles() + "," + rhs.getNumEles() + ") when trying to add vectors!");
