...
public abstract class SumType extends PostfixMathCommand implements CallbackEvaluationI {
	...
	public Object evaluate(Node node, EvaluatorI pv) throws ParseException {
		...
		if (minObj instanceof Number)
			...
			throw new ParseException(name + ": third argument (min) should evaluate to a number it is " + minObj.toString());
		...
		if (maxObj instanceof Number)
			...
			throw new ParseException(name + ": forth argument (max) should evaluate to a number it is " + minObj.toString());

		if (min > max)
			throw new ParseException(name + ": min value should be smaller than max value they are " + min + " and " + max + ".");
		...
		if (numParams == 5) {
			...
			if (incObj instanceof Number)
				...
				throw new ParseException(name + ": fifth argument (steps) should evaluate to a number it is " + minObj.toString());
