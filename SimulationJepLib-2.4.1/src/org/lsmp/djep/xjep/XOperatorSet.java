...
public class XOperatorSet extends OperatorSet {
	...
	private void annotateOperators(OperatorSet o) {
		...
		setPrecedenceTable(new Operator[][] { { OP_UMINUS }, { OP_NOT }, { OP_POWER }, { OP_MULTIPLY, OP_DIVIDE, OP_MOD, OP_DOT, OP_CROSS },
				{ OP_ADD, OP_SUBTRACT }, { OP_LT, OP_LE }, { OP_GT, OP_GE }, { OP_EQ }, { OP_NE }, { OP_AND }, { OP_OR }, { OP_ASSIGN }, });
		// printOperators();
		...
		// -(a+b) -> (-a) +(-b)
		((XOperator) OP_UMINUS).setDistributiveOver(OP_ADD);
		// -(a-b) -> (-a) - (-b)
		((XOperator) OP_UMINUS).setDistributiveOver(OP_SUBTRACT);

		// a*(b+c) -> a*b + a*c
		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_ADD);
		// a*(b-c) -> a*b - a*c
		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_SUBTRACT);
		// a*(-b) -> -(a*b)
		((XOperator) OP_MULTIPLY).setDistributiveOver(OP_UMINUS);
	}
	 ...
	 * 
	 * <pre>
	 * setPrecedenceTable(new Operator[][] { { OP_UMINUS }, { OP_NOT }, { OP_MUL, OP_DIV, OP_MOD }, { OP_PLUS, OP_MINUS }, { OP_LT, OP_LE }, { OP_GT, OP_GE },
