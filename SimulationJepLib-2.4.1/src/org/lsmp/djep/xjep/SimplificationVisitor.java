...
public class SimplificationVisitor extends DoNothingVisitor {
	...
	public Node simplifyTripple(XOperator op, Node lhs, Node rhs) throws ParseException {
		...
		if (tu.isConstant(lhs) && tu.isBinaryOperator(rhs)) {
			...
			if (tu.isConstant(rhsChild1)) {
				...
				// ~(2 @ x)
				{
					Node newnode = simplifyBuiltOperatorNode(rhsOp, nf.buildConstantNode(op, lhs, rhsChild1), simplifyBuiltOperatorNode(op, lhs, rhsChild2));
					return newnode;
				}
			}
			...
			if (tu.isConstant(rhsChild2)) {
				...
				if (op.isDistributiveOver(rhsRoot)) {
					Node newnode = simplifyBuiltOperatorNode(rhsOp, simplifyBuiltOperatorNode(op, lhs, rhsChild1), nf.buildConstantNode(op, lhs, rhsChild2));
					return newnode;
				}
			}
		}
		...
		if (tu.isBinaryOperator(lhs) && tu.isConstant(rhs)) {
			...
			if (tu.isConstant(lhsChild1)) {
				...
				if (op.isDistributiveOver(lhsRoot)) {
					Node newnode = simplifyBuiltOperatorNode(lhsOp, nf.buildConstantNode(op, lhsChild1, rhs), simplifyBuiltOperatorNode(op, lhsChild2, rhs));
					return newnode;
				}
			}
			...
			if (tu.isConstant(lhsChild2)) {
				...
				if (rootOp == lhsRoot && rootOp.isAssociative()) {
					...
					Node newnode = simplifyBuiltOperatorNode(lhsOp, lhsChild1, nf.buildConstantNode(op2, lhsChild2, rhs));
					return newnode;
				}

				// (x + ~2) * 3 -> (x*3) + ~(2*3)
				if (op.isDistributiveOver(lhsRoot)) {
					Node newnode = simplifyBuiltOperatorNode(lhsOp, simplifyBuiltOperatorNode(op, lhsChild1, rhs), nf.buildConstantNode(op, lhsChild2, rhs));
					return newnode;
				}
			}
		}
	...
	public Node simplifySubtract(Node lhs, Node rhs) throws ParseException {
		...
		// 0 - x -> -(x)
		if (tu.isZero(lhs)) {
			return nf.buildOperatorNode(opSet.getUMinus(), rhs);
		}
	...
	public Node simplifyPower(Node child1, Node child2) throws ParseException {
		...
		if (tu.isConstant(child2) && tu.getOperator(child1) == opSet.getPower()) {
			if (tu.isConstant(child1.jjtGetChild(1))) {
				/* (x^3)^4 -> x^(3*4) */
				return nf.buildOperatorNode(opSet.getPower(), child1.jjtGetChild(0), nf.buildConstantNode(opSet.getMultiply(), child1.jjtGetChild(1), child2));
			}
		}
		return null;
		// return nf.buildOperatorNode(((ASTOpNode)
