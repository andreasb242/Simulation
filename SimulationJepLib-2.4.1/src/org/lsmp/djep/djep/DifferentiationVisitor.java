...
public class DifferentiationVisitor extends DeepCopyVisitor {
	 ...
	 */
	protected DifferentiationVisitor(DJep jep) {
		this.globalDJep = jep;

	}

	/** The set of all differentation rules indexed by name of function. */
	private Hashtable<String, DiffRulesI> diffRules = new Hashtable<String, DiffRulesI>();
	 ...
	 * stream.
	 */
	private void printDiffRules(PrintStream out) {
		out.println("Standard Functions and their derivatives");
		for (Entry<String, PostfixMathCommandI> e : globalDJep.getFunctionTable().entrySet()) {
			String key = e.getKey();
			PostfixMathCommandI value = e.getValue();
		...
		for (Entry<String, DiffRulesI> e : diffRules.entrySet()) {
			String key = e.getKey();
			DiffRulesI rule = e.getValue();
	 ...
	 */
	protected Node differentiate(Node node, String var, DJep djep) throws ParseException, IllegalArgumentException {
		this.localDJep = djep;
