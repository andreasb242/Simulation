...
public class PowerDiffRule implements DiffRulesI {
	...
	public Node differentiate(ASTFunNode node, String var, Node[] children, Node[] dchildren, DJep djep) throws ParseException {
		...
		if (tu.isConstant(children[1])) {
			...
			return nf.buildOperatorNode(op.getMultiply(), djep.deepCopy(children[1]), nf.buildOperatorNode(op.getMultiply(), nf.buildOperatorNode(
					op.getPower(), djep.deepCopy(children[0]), nf.buildOperatorNode(op.getSubtract(), djep.deepCopy(children[1]), nf.buildConstantNode(tu
							.getONE()))), dchildren[0]));
			...
			// z * y^(z-1) * diff(y,x)
					...
					nf.buildOperatorNode(op.getMultiply(), // + y^z * ln(z) *
							...
							nf.buildOperatorNode(op.getMultiply(), nf.buildOperatorNode(op.getPower(), // y^z
									djep.deepCopy(children[0]), djep.deepCopy(children[1])), djep.getNodeFactory().buildFunctionNode("ln", funTab.get("ln"), // ln(z)
