 ...
 * unable to be loaded any text specified via the <code>ALT</code> attribute
 * will be rendered.
 * <p>
 ...
 * @since 1.4
 */
public class InternImageView extends View {
	/**
	 ...
	 */
	public InternImageView(Element elem) {
		super(elem);
	...
	public void paint(Graphics g, Shape a) {
		...
		if (image != null) {
			...
			if (view != null && ((state & WIDTH_FLAG) == 0 || width > DEFAULT_WIDTH)) {
				...
				Rectangle altRect = new Rectangle(rect.x + leftInset + DEFAULT_WIDTH, rect.y + topInset, rect.width - leftInset - rightInset - DEFAULT_WIDTH,
	...
	private void paintBorder(Graphics g, Rectangle rect) {
		...
		if ((borderSize > 0 || image == null) && color != null) {
			...
			for (int counter = 0; counter < n; counter++) {
				g.drawRect(rect.x + xOffset + counter, rect.y + yOffset + counter, rect.width - counter - counter - xOffset - xOffset - 1, rect.height
						- counter - counter - yOffset - yOffset - 1);
			}
		}
	}
	...
	public void setSize(float width, float height) {
		...
		if (getImage() == null) {
			...
			if (view != null) {
				view.setSize(Math.max(0f, width - (float) (DEFAULT_WIDTH + leftInset + rightInset)), Math.max(0f, height - (float) (topInset + bottomInset)));
			}
		}
	}
	 ...
	 * instead of invoking <code>loadImage</code> or
	...
	/**
	 * Loads the image from the URL <code>getImageURL</code>. This should only
	 * be invoked from <code>refreshImage</code>.
	 */
	private void loadImage() {
		// Versuche interne Bilder zu laden
		String id = (String) getElement().getAttributes().getAttribute(HTML.Attribute.ID);
		if (id != null) {
			if(id.startsWith("$")) {
	...
	private class ImageHandler implements ImageObserver {
		...
		public boolean imageUpdate(Image img, int flags, int x, int y, int newWidth, int newHeight) {
			...
			if ((flags & (ABORT | ERROR)) != 0) {
				repaint(0);
				synchronized (InternImageView.this) {
					if (image == img) {
