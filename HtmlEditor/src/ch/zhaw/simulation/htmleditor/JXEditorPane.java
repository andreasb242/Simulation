...
public class JXEditorPane extends JEditorPane implements Targetable {
	...
	/*
	 * These next 3 actions are part of a *HACK* to get cut/copy/paste support
	 * working in the same way as find, undo and redo. in JTextComponent the
	 * cut/copy/paste actions are _not_ added to the ActionMap. Instead, a
	 * default "transfer handler" system is used, apparently to get the text
	 * onto the system clipboard. Since there aren't any CUT/COPY/PASTE actions
	 * in the JTextComponent's action map, they cannot be referenced by the
	 * action framework the same way that find/undo/redo are. So, I added the
	 * actions here. The really hacky part is that by defining an Action to go
	 * along with the cut/copy/paste keys, I loose the default handling in the
	 * cut/copy/paste routines. So, I have to remove cut/copy/paste from the
	 * action map, call the appropriate method (cut, copy, or paste) and then
	 * add the action back into the map. Yuck!
	 */
	...
	/**
	 * Creates a new <code>JXEditorPane</code>. The document model is set to
	 * <code>null</code>.
	 */
	public JXEditorPane() {
		init();
	...
	/**
	 * Creates a <code>JXEditorPane</code> based on a string containing a URL
	 * specification.
	 * 
	 * @param url
	 *            the URL
	 * @exception IOException
	 *                if the URL is <code>null</code> or cannot be accessed
	 */
	...
	/**
	 * Creates a <code>JXEditorPane</code> that has been initialized to the
	 * given text. This is a convenience constructor that calls the
	...
	public class DocumentSearchable implements Searchable {
		...
		/**
		 * checks if the searchString should be interpreted as empty. here:
		...
		/**
		 * Search from same startIndex as the previous search. Checks if the
		 * match is different from the last (either extended/reduced) at the
		 * same position. Returns true if the current match result represents a
		 * different match than the last, false if no match or the same.
		 * 
		 * @param pattern
		 * @param start
		 * @return true if the current match result represents a different match
		 ...
		 * @param matcher
		 * @param useFirst
		 *            whether or not to return after the first match is found.
	...
	/**
	 * Listens to the caret placement and adjusts the editing properties as
	 * appropriate.
	 * 
	...
	/**
	 * Handles sloppy HTML. This implementation currently only looks for tags
	 * that have a / at the end (self-closing tags) and fixes them to work with
	 * the version of HTML supported by HTMLEditorKit
	 * <p>
	 * 
	 * @author rbair
	 */
